...
public abstract class WebDriverInterface extends AutomationInterface {
	...
	public WebDriverPause getPause() {
		long timeoutS = Long.parseLong(candybean.config.getValue("perf.explicit.wait.timeout.seconds", "15"));
		long pollingS = Long.parseLong(candybean.config.getValue("perf.explicit.wait.polling.seconds", "5"));
		return new WebDriverPause(wd, timeoutS, pollingS);
	}
